...
io.on('connection', (socket) => {
    ...
    socket.on('host-join', (data) => {
        ...
        MongoClient.connect(url, function (err, db) {
            if (err) throw err;
            var dbo = db.db("quiznowDB");
            var query = { id: parseInt(data.id) };
            dbo.collection('quiznowGames').find(query).toArray(function (err, result) {
                if (err) throw err;
    ...
    // socket.on('getTotalNrOfQuestions', (data) => {
    //     console.log("getting total nr of questions for this game")
    ...
    //             dbo.collection("quiznowGames").find(query).toArray(function (err, res) {
    //                 if (err) throw err;
    ...
    //             });
    //         });
    //         console.log("return nr of questions")
    //         socket.emit("receivedTotalNrOfQuestions", nrOfQuestions)
    //     }
    // });
    ...
    socket.on('requestDbNames', function () {
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            var dbo = db.db('quiznowDB');
            dbo.collection("quiznowGames").find().toArray(function (err, res) {
                if (err) throw err;
    ...
    socket.on('newQuiz', function (data) {
        MongoClient.connect(url, function (err, db) {
            if (err) throw err;
            var dbo = db.db('quiznowDB');
            dbo.collection('quiznowGames').find({}).toArray(function (err, result) {
                if (err) throw err;
                ...
                var game = data;
                dbo.collection("quiznowGames").insertOne(game, function (err, res) {
                    if (err) throw err;
                    db.close();
                });
                db.close();
